...
public class MailPage {
	...
	//	drv.switchTo().frame(drv.findElement(obj.getBy("frmcompose")));
		wait.until(ExpectedConditions.presenceOfElementLocated(obj.getBy("btncompose")));
		drv.findElement(obj.getBy("btncompose")).click();
		
	}

	
	public void sendemail(WebDriver drv, String to, String sub , String content) throws InterruptedException
	{
		WebDriverWait wait ;
		wait = new WebDriverWait(drv, 500);
		wait.until(ExpectedConditions.presenceOfElementLocated(obj.getBy("divcontent")));
		try {
			Thread.sleep(2000);
	...
	public void clickprimarytab(WebDriver drv)
	{
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(obj.getBy("tabprimary")));
	...
	public void clickemailwithsubjectline(WebDriver drv,String subjecttext)
	{
		...
		String origval = prstvalue;
		System.out.println("Old Vlaue"+origval);
		prstvalue = prstvalue.replace("<placeholder>", subjecttext);
		System.out.println("New Vlaue: "+prstvalue);
		hlp.updatePropertyValue("linkmymail", prstvalue);
		
		wait.until(ExpectedConditions.presenceOfElementLocated(obj.getBy("linkmymail")));
		try {
			Thread.sleep(2000);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
		 ...
		 hlp.updatePropertyValue("linkmymail", origval);
	...
	//	WebDriverWait wait ;
	//	wait = new WebDriverWait(drv, 500);
	//	wait.until(ExpectedConditions.presenceOfElementLocated(obj.getBy("emailbodytext")));
		List<WebElement>  allelem = drv.findElements(obj.getBy("emailbodytext"));
		String acttext ="";
		for(WebElement elem :allelem){
			if (elem.isDisplayed())
			{
				acttext= elem.getText();
			}
		}
		
		if (acttext.equalsIgnoreCase(subjecttext) )
		{
			System.out.println("*************Test Pass:   for Email found******** Expected:  [ " +subjecttext+"] Actual:  ["+acttext+"]");
		}
		else
		{
			System.out.println("*************Test Fail:   for Email found******** Expected: [" +subjecttext+" ]Actual:  ["+acttext+"]");
		}
		String timestampfilename = new SimpleDateFormat("dd_MMM_yyyy_HH_mm_ss").format(new Date());
		
		try {
			hlp.CaptureScreesnhot(drv, ScreenshotDirectory +"\\Image_"+timestampfilename+".jpg");
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public void deletemail(WebDriver drv) throws InterruptedException, AWTException
	{
	...
	public void logout(WebDriver drv)
	{
		...
		wait.until(ExpectedConditions.presenceOfElementLocated(obj.getBy("btnsignout")));
	...
	public  void clickdel(WebDriver driver) throws InterruptedException{
		...
		for(WebElement element:elements){
			...
			//	System.out.println("Clicking Button not hiddden "+i);
				...
				{
					System.out.println("Element does not have attri as data-tooltip "+e);
				}
				if (val != null )
				{
